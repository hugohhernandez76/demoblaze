...
public class TotalAmountCart extends setUp{
    ...
    void validatetotalamount() {
        ...
        if (driver == null) {
            ...
            //Validate Total word is displayed
            String total = driver.findElement(By.xpath("//h2[contains(text(),'Total')]")).getText();
            Assert.assertEquals(total, "Total", "Total Word not displayed");

            //Validating the sum amount of the 3 items from the cart amount should add upt to $1860.00
            String amount = driver.findElement(By.xpath("//h3[@id='totalp']")).getText();
            Assert.assertTrue(amount.contains("1860"));
